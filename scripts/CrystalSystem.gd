...
enum CrystalType {
	...
	TECH_CRYSTAL,      # Технологические кристаллы
	BIO_CRYSTAL,       # Биологические кристаллы
	PSI_CRYSTAL        # Пси-кристаллы
}
...
func create_crystal_field():
	...
	var crystal_configs = [
		...
		{"name": "Главный Кристалл Юг", "pos": Vector3(0, 1, 20), "type": CrystalType.MAIN_CRYSTAL, "capacity": 10000, "regen": 0, "radius": 8.0},
		{"name": "Главный Кристалл Север", "pos": Vector3(0, 1, -20), "type": CrystalType.MAIN_CRYSTAL, "capacity": 10000, "regen": 0, "radius": 8.0},
		
		# === ЭНЕРГЕТИЧЕСКИЕ КРИСТАЛЛЫ (основные ресурсы) ===
		{"name": "Энергокристалл Левый", "pos": Vector3(-15, 1, 10), "type": CrystalType.ENERGY_CRYSTAL, "capacity": 5000, "regen": 10, "radius": 6.0},
		{"name": "Энергокристалл Правый", "pos": Vector3(15, 1, 10), "type": CrystalType.ENERGY_CRYSTAL, "capacity": 5000, "regen": 10, "radius": 6.0},
		{"name": "Энергокристалл Верх-Лево", "pos": Vector3(-15, 1, -10), "type": CrystalType.ENERGY_CRYSTAL, "capacity": 5000, "regen": 10, "radius": 6.0},
		{"name": "Энергокристалл Верх-Право", "pos": Vector3(15, 1, -10), "type": CrystalType.ENERGY_CRYSTAL, "capacity": 5000, "regen": 10, "radius": 6.0},
		
		# === ТЕХНОЛОГИЧЕСКИЕ КРИСТАЛЛЫ (центральные) ===
		{"name": "Техно-Кристалл Центр", "pos": Vector3(0, 1, 0), "type": CrystalType.TECH_CRYSTAL, "capacity": 1000, "regen": 2, "radius": 5.0},
		{"name": "Техно-Кристалл Лево", "pos": Vector3(-8, 1, 0), "type": CrystalType.TECH_CRYSTAL, "capacity": 800, "regen": 2, "radius": 4.0},
		{"name": "Техно-Кристалл Право", "pos": Vector3(8, 1, 0), "type": CrystalType.TECH_CRYSTAL, "capacity": 800, "regen": 2, "radius": 4.0},
		
		# === БИОЛОГИЧЕСКИЕ КРИСТАЛЛЫ ===
		{"name": "Био-Кристалл Юг-Лево", "pos": Vector3(-10, 1, 15), "type": CrystalType.BIO_CRYSTAL, "capacity": 2000, "regen": 5, "radius": 4.5},
		{"name": "Био-Кристалл Юг-Право", "pos": Vector3(10, 1, 15), "type": CrystalType.BIO_CRYSTAL, "capacity": 2000, "regen": 5, "radius": 4.5},
		{"name": "Био-Кристалл Север-Лево", "pos": Vector3(-10, 1, -15), "type": CrystalType.BIO_CRYSTAL, "capacity": 2000, "regen": 5, "radius": 4.5},
		{"name": "Био-Кристалл Север-Право", "pos": Vector3(10, 1, -15), "type": CrystalType.BIO_CRYSTAL, "capacity": 2000, "regen": 5, "radius": 4.5},
		
		# === ПСИ-КРИСТАЛЛЫ (редкие) ===
		{"name": "Пси-Кристалл Альфа", "pos": Vector3(-5, 1, 5), "type": CrystalType.PSI_CRYSTAL, "capacity": 500, "regen": 1, "radius": 3.0},
		{"name": "Пси-Кристалл Бета", "pos": Vector3(5, 1, -5), "type": CrystalType.PSI_CRYSTAL, "capacity": 500, "regen": 1, "radius": 3.0},
	]
...
func get_capture_time(type: CrystalType) -> float:
	match type:
		...
		CrystalType.TECH_CRYSTAL:
			return 8.0   # Захват: 8 сек → Освобождение: 12 сек
		CrystalType.BIO_CRYSTAL:
			return 4.0   # Захват: 4 сек → Освобождение: 6 сек
		CrystalType.PSI_CRYSTAL:
			return 6.0   # Захват: 6 сек → Освобождение: 9 сек
		_:
			return 5.0
...
func create_crystal_visual(crystal: Dictionary):
	...
	match crystal.type:
		...
		CrystalType.PSI_CRYSTAL:
			var box = BoxMesh.new()
			box.size = Vector3(crystal.control_radius * 0.8, crystal.control_radius * 1.4, crystal.control_radius * 0.8)
			mesh_instance.mesh = box
	...
	match crystal.type:
		...
		CrystalType.TECH_CRYSTAL:
			material.albedo_color = Color(0.7, 0.7, 1.0, 0.9)  # Серебристо-синий
			material.emission = Color(0.5, 0.5, 0.8)
		CrystalType.BIO_CRYSTAL:
			material.albedo_color = Color(0.0, 1.0, 0.3, 0.9)  # Ярко-зеленый
			material.emission = Color(0.0, 0.8, 0.2)
		CrystalType.PSI_CRYSTAL:
			material.albedo_color = Color(1.0, 0.0, 1.0, 0.9)  # Пурпурный
			material.emission = Color(0.8, 0.0, 0.8)
...
func get_crystal_info_text(crystal: Dictionary) -> String:
	...
	match crystal.type:
		...
		CrystalType.TECH_CRYSTAL:
			type_name = "🔬 ТЕХНОЛОГИИ"
			bonus_info = "Исследования"
		CrystalType.BIO_CRYSTAL:
			type_name = "🌿 БИО-МАССА"
			bonus_info = "Растет со временем"
		CrystalType.PSI_CRYSTAL:
			type_name = "🧠 ПСИ-ЭНЕРГИЯ"
			bonus_info = "Особые способности"
...
func apply_crystal_regeneration_effects(crystal: Dictionary):
	match crystal.type:
		CrystalType.BIO_CRYSTAL:
			# Био-кристаллы могут расти и увеличивать максимальную емкость
			if crystal.growth_level < 2.0 and randf() < 0.05:  # 5% шанс роста
				...
				print("🌱 Био-кристалл ", crystal.id, " вырос! Новая емкость: ", crystal.max_capacity)
				
		CrystalType.PSI_CRYSTAL:
			# Пси-кристаллы могут стать нестабильными
			if randf() < 0.1:  # 10% шанс нестабильности
				crystal.instability += 0.1
				if crystal.instability > 1.0:
					trigger_psi_anomaly(crystal)

func trigger_psi_anomaly(crystal: Dictionary):
	print("🌀 Пси-аномалия в кристалле ", crystal.id, "!")
	
	var anomaly_type = randi() % 3
	match anomaly_type:
		0:  # Энергетический всплеск
			crystal.current_capacity = crystal.max_capacity * 2  # Удваиваем энергию
			show_notification("⚡ Энергетический всплеск в пси-кристалле!")
		1:  # Телепортация ближайших юнитов
			teleport_nearby_units(crystal.position, 15.0)
			show_notification("🌀 Пси-кристалл телепортировал юнитов!")
		2:  # Временное отключение соседних кристаллов
			disable_nearby_crystals(crystal.position, 20.0, 30.0)
			show_notification("💫 Пси-шторм отключил соседние кристаллы!")
	
	crystal.instability = 0.0  # Сбрасываем нестабильность
...
func apply_crystal_effects(crystal: Dictionary):
	...
	match crystal.type:
		CrystalType.MAIN_CRYSTAL:
			...
			add_resource(crystal_owner, "crystals", amount / 2)
		...
		CrystalType.TECH_CRYSTAL:
			# Технологические кристаллы дают исследовательские очки
			add_resource(crystal_owner, "crystals", amount)
			boost_tech_research(crystal_owner)
			
		CrystalType.BIO_CRYSTAL:
			# Био-кристаллы дают био-массу и могут лечить юнитов
			add_resource(crystal_owner, "crystals", amount)
			heal_nearby_units(crystal.position, crystal_owner, 10.0)
			
		CrystalType.PSI_CRYSTAL:
			# Пси-кристаллы дают пси-энергию для способностей
			add_resource(crystal_owner, "crystals", amount * 2)
			reduce_ability_cooldowns(crystal_owner)
...
func get_resource_rate(type: CrystalType) -> int:
	match type:
		...
		CrystalType.PSI_CRYSTAL:
			return 8   # Пси-кристаллы - редкие и мощные
		_:
			return 0
...
func apply_capture_effects(crystal: Dictionary, old_owner: String, new_owner: String):
	...
	match crystal.type:
		...
		CrystalType.PSI_CRYSTAL:
			# Пси-кристалл сбрасывает кулдауны способностей
			reset_all_cooldowns(new_owner)
			show_notification("🧠 Пси-кристалл сбросил кулдауны способностей!")
...
func restore_original_crystal_color(crystal: Dictionary, material: StandardMaterial3D):
	match crystal.type:
		...
		CrystalType.TECH_CRYSTAL:
			material.albedo_color = Color(0.7, 0.7, 1.0, 0.9)
		CrystalType.BIO_CRYSTAL:
			material.albedo_color = Color(0.0, 1.0, 0.3, 0.9)
		CrystalType.PSI_CRYSTAL:
...
func force_capture_crystal(crystal_id: int, owner: String):
	...
	crystal_captured.emit(crystal.id, owner, crystal.type)
	print("💎 Кристалл ", crystal_id, " принудительно захвачен командой ", owner)
	return true
...
func apply_liberation_effects(crystal: Dictionary, previous_owner: String, liberating_team: String):
	...
	match crystal.type:
		...
		CrystalType.PSI_CRYSTAL:
			# Пси-кристалл может вызвать хаос при освобождении
			show_notification("🧠 Пси-кристалл освобожден! Пси-энергия в хаосе!")
			trigger_liberation_psi_effect(crystal, previous_owner, liberating_team)

# НОВАЯ ФУНКЦИЯ: Специальные пси-эффекты при освобождении
func trigger_liberation_psi_effect(crystal: Dictionary, previous_owner: String, liberating_team: String):
	print("🌀 Пси-эффект освобождения кристалла ", crystal.id, " от ", previous_owner, " к ", liberating_team)
	
	var effect_type = randi() % 3
	match effect_type:
		0:  # Пси-взрыв оглушает ближайших врагов
			stun_nearby_enemies(crystal.position, previous_owner, 15.0, 5.0)
			show_notification("💥 Пси-взрыв оглушил врагов!")
		1:  # Пси-волна лечит освободителей
			heal_nearby_allies(crystal.position, liberating_team, 15.0, 50)
			show_notification("💚 Пси-волна лечит союзников!")
		2:  # Пси-хаос меняет сторону случайных врагов
			convert_nearby_enemies(crystal.position, previous_owner, liberating_team, 10.0)
			show_notification("🌀 Пси-хаос обратил врагов!")

# Вспомогательные функции для пси-эффектов - используем параметры
func stun_nearby_enemies(pos: Vector3, enemy_team_name: String, search_radius: float, effect_duration: float):
	print("💥 Оглушение врагов ", enemy_team_name, " в радиусе ", search_radius, " на ", effect_duration, " секунд от ", pos)
	# TODO: Реализовать оглушение юнитов

func heal_nearby_allies(pos: Vector3, ally_team_name: String, search_radius: float, heal_amount: int):
	print("💚 Лечение союзников ", ally_team_name, " в радиусе ", search_radius, " на ", heal_amount, " HP от ", pos)
	# TODO: Реализовать лечение юнитов

func convert_nearby_enemies(pos: Vector3, enemy_team_name: String, new_team_name: String, search_radius: float):
	print("🌀 Обращение врагов ", enemy_team_name, " в ", new_team_name, " в радиусе ", search_radius, " от ", pos)
	# TODO: Реализовать смену команды юнитов
...
func get_crystal_type_name(crystal_type: int) -> String:
	match crystal_type:
		...
		CrystalType.TECH_CRYSTAL: return "TECH_CRYSTAL"
		CrystalType.BIO_CRYSTAL: return "BIO_CRYSTAL"
		CrystalType.PSI_CRYSTAL: return "PSI_CRYSTAL"
		_: return "UNKNOWN" 
